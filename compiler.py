    ...
    def operator(self, left, op, right, store_dst=None):
        ...
        if opper:
            ...
            if op.value == '<':
                ...
                jmp_true = self._push_conditional_eq_jump_inst(r0, r1, label=None, eq=True)
                self._push_copy_inst(src='0', dst=dst)
                jmp_false = self._push_jump_inst()
                self._insert_label(jmp_true)
                self._push_copy_inst(src='1', dst=dst)
                self._insert_label(jmp_false)
            elif op.value == '!=':
                jmp_true = self._push_conditional_eq_jump_inst(r0, r1, label=None, eq=False)
                self._push_copy_inst(src='0', dst=dst)
                jmp_false = self._push_jump_inst()
                self._insert_label(jmp_true)
                self._push_copy_inst(src='1', dst=dst)
